...
- **Player lookup** by tag. Pulls the player's current clan, plus any clan their recent battle log shows them fighting for, and charts those weeks. So a recruit who just joined still shows their history from the clan they came from.
...
- `api/lookup.php?tag=%23TAG` - one player's history. Calls the live API for the player, their battle log, their current clan and up to three clans seen in recent battles, cached 15 minutes per endpoint.
